...
/**
    ...
    /**
     * @Route("/update", name="ajouter",methods={"GET"})
     */
   /* public function update(Request $request,ActualiteRepository $actrep,GameRepository $gamerep,PersonneRepository $perep){
       ...
       // $actualites->setIdMatch($gamerep->find((int)$request->query->get('id_match')));
        ...
        return new Response('Actualité modifié');
    }*/
     ...
     */
   /* public function delete(ActualiteRepository $repository ,SerializerInterface $serializer)
    {
        $actualites=$courRepository->find($request->get('id'));

        $en->remove($actualites);
        $en->flush();
//        $encoders=[new JsonEncoder()];
//        $normalizer =[new ObjectNormalizer()];
//        $serializer=new \Symfony\Component\Serializer\Serializer($normalizer,$encoders);
